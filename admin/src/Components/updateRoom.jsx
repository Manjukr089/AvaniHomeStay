...
const UpdateRoom = () => {
  ...
  //     if (roomData.length > 0) {
  //       const { id, type, name, description, capacity, size, price, breakfast, images } = roomData[0];
  //       setType(type);
  //       setName(name);
  ...
  useEffect(() => {
    async function fetchRoomData() {
      try {
        ...
        console.log('Fetched room data:', data); // Log the fetched data
        if (Array.isArray(data) && data.length > 0) {
          const { id, type, name, description, capacity, size, price, breakfast, images } = data[0];
          setType(type);
          setName(name);
          setdescription(description);
          setcapacity(capacity);
          ...
          images.forEach((image, index) => {
            roomData.append(`image${index + 1}`, image);
          });
          // setImage1(images[0]);
          // setImage2(images[1]);
          // setImage3(images[2]);
  ...
  function getRoom(arg) {
    const idiRooms = state[0][0].rooms.map((item) => item);
    return idiRooms.filter((roomItem) => roomItem.slug === arg);
  }

  async function updateRoomMySQL() {
    try {
      const response = await axios.put(`http://localhost:3001/rooms/${id}`, {
        name,
        type,
        price,
        size,
